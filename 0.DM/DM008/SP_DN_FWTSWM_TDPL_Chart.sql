USE [ROS]
GO
/****** Object:  StoredProcedure [dbo].[SP_DN_FWTSWM_TDPL_Chart]    Script Date: 4/19/2023 7:33:49 PM ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[SP_DN_FWTSWM_TDPL_Chart]
as
---------------------------- Output Result ----------------------------
...
select (
	select tb.Workline
	,SUM(tb.PlanQty) PlanQty
	,SUM(ISNULL(t2.TotalQty,0)) ExportQty
	...
	group by tb.Workline
	order by len(tb.Workline), tb.Workline
	for json path
) as [Value]
--[SP_DN_FWTSWM_TDPL_Chart]